...
public class BreakMultipleWordsWithNoSpaceIntoSpace {
	...
	/**
	 * Recursive and slow version of breaking word problem. If no words can be
	 * formed it returns null
	 */
	public String breakWord(char[] str, int low, Set<String> dictionary) {
		...
		for (int i = low; i < str.length; i++) {
			...
			if (dictionary.contains(buff.toString())) {
				final String result = breakWord(str, i + 1, dictionary);
				if (result != null) {
	...
	/**
	 * Dynamic programming version for breaking word problem. It returns null string
	 * if string cannot be broken into multipe words such that each word is in
	 * dictionary. Gives preference to longer words over splits e.g peanutbutter
	 * with dict{pea nut butter peanut} it would result in peanut butter instead of
	 * pea nut butter.
	 */
	public String breakWordDP(String word, Set<String> dict) {
		final int T[][] = new int[word.length()][word.length()];
		...
		for (int l = 1; l <= word.length(); l++) {
			for (int i = 0; i < word.length() - l + 1; i++) {
				final int j = i + l - 1;
				final String str = word.substring(i, j + 1);
	...
	/**
	 * Prints all the words possible instead of just one combination. Reference
	...
	private List<String> wordBreakUtil(String s, Set<String> dict, Map<Integer, List<String>> dp, int start, int max) {
		...
		for (int i = start; i < start + max && i < s.length(); i++) {
			final String newWord = s.substring(start, i + 1);
			if (!dict.contains(newWord)) {
				continue;
			}
			final List<String> result = wordBreakUtil(s, dict, dp, i + 1, max);
			for (final String word : result) {
				final String extraSpace = word.length() == 0 ? "" : " ";
	...
	/**
	 * Check if any one solution exists. https://leetcode.com/problems/word-break/
	 */
	public boolean wordBreakTopDownOneSolution(String s, Set<String> wordDict) {
		final Map<Integer, Boolean> dp = new HashMap<>();
		int max = 0;
		for (final String s1 : wordDict) {
			max = Math.max(max, s1.length());
		}
	...
	public static void main(String args[]) {
		...
		dictionary.add("play");
		dictionary.add("to");
		final String str = "Ihadliketoplay";
		final BreakMultipleWordsWithNoSpaceIntoSpace bmw = new BreakMultipleWordsWithNoSpaceIntoSpace();
		final String result1 = bmw.breakWordDP(str, dictionary);
