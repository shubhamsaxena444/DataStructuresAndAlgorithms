...
public class OptimalTreeSearch {
	...
	private int minCostRec(int input[], int freq[], int low, int high, int level) {
		...
		for (int i = low; i <= high; i++) {
			final int val = minCostRec(input, freq, low, i - 1, level + 1)
					+ minCostRec(input, freq, i + 1, high, level + 1) + level * freq[i];
			if (val < min) {
				min = val;
			}
		}
	...
	public int minCost(int input[], int freq[]) {
		final int T[][] = new int[input.length][input.length];
		...
		for (int l = 2; l <= input.length; l++) {
			for (int i = 0; i <= input.length - l; i++) {
				...
				final int sum = getSum(freq, i, j);

				for (int k = i; k <= j; k++) {
					final int val = sum + (k - 1 < i ? 0 : T[i][k - 1]) + (k + 1 > j ? 0 : T[k + 1][j]);
					if (val < T[i][j]) {
						T[i][j] = val;
					}
				}
	...
	public static void main(String args[]) {
		final int input[] = { 10, 12, 16, 21 };// {10,12,20,35,46};
		final int freq[] = { 4, 2, 6, 3 };// {34,8,50,21,16};
		final OptimalTreeSearch ots = new OptimalTreeSearch();
